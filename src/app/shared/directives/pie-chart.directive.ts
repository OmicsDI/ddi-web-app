...
export class PieChartDirective implements OnInit {
    ...
    add_tooltip(element, data, i, tool_tip, root) {
        if (i < data.length) {
            element.on('mouseover', function () {
                    tool_tip.transition()
                        .duration(200)
                        .style('opacity', .9);
                    const mouse_coords = d3.mouse(root);
                    tool_tip.html(data[i]['label'] + ': ' +
                        d3.format(',d')(data[i]['score']))
    ...
    ngOnInit(): void {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const leafs = contributed_scale / (1 / 12);
            data[i]['leafs'] = (leafs > 0 && leafs < 1) ? 1 : Math.round(leafs);
            for (let j = 0; j < data[i]['leafs']; j++) {
                has_color.push(data[i]);
            }
        }
        for (let i = 0; i < 12; i++) {
            ...
            if (i < has_color.length) {
                gradient.append('stop').attr('offset', '100%')
                    .attr('style', 'stop-color:' + has_color[i]['color'] + ';stop-opacity:1');
            } else {
        ...
        let path = graph.append('path').attr('d', 'm 378.9895761619361,264.3949755505212 c -5.7581064046051,-9.760471988086' +
            ...
            .attr('fill', 'url(#' + self.dataset.id + '_gradient_' + color_index + ')')
            .attr('style', 'stroke:none');
        self.add_tooltip(path, has_color, color_index++, tool_tip, self.el.nativeElement);
        ...
        path = graph.append('path').attr('d', 'm 421.3837824508547,420.2462819635038 c 11.3318698969114,-0.1064304300389 ' +
        ...
        path = graph.append('path').attr('d', 'm 379.6725888638378,444.5395756745965 c 9.8669024181249,5.5737634923063 ' +
        ...
        path = graph.append('path').attr('d', 'm 331.4029887397643,444.7225883764912 c 5.758106404605,9.7604719880859 ' +
        ...
        path = graph.append('path').attr('d', 'm 289.5087824508529,420.7462819635105 c 0.106430430039,11.3318698969113 ' +
        ...
        path = graph.append('path').attr('d', 'm 265.2154887397601,379.0350883764934 c -5.5737634923061,9.8669024181251 ' +
        ...
        path = graph.append('path').attr('d', 'm 265.0324760378657,330.7654882524203 c -9.760471988086,5.7581064046052' +
        ...
        path = graph.append('path').attr('d', 'm 289.0087824508462,288.8712819635089 c -11.3318698969114,0.106430430039 ' +
        ...
        path = graph.append('path').attr('d', 'm 330.7199760378628,264.5779882524163 c -9.8669024181249,-5.5737634923063 ' +
