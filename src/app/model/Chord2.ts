...
export class Chord2 {
  ...
  public init(selection: any) {
    ...
    selection.each(function (d, i) {
      ...
      selection.append("g")
        ...
        .on("click", self.fade(selection, .0))
        .on("mouseover", self.fade(selection, .2))
      ...
      let groupsNo = self.cgroups().length;
  ...
  cchords() {
    if (!this.chords) this.relayout();
    return this.chords;
  }

  cgroups() {
    if (!this.groups) this.relayout();
    return this.groups;
  };
  ...
  relayout() {
    ...
    if (self.sortSubgroups) {
      subgroupIndex.forEach(function (d, i) {
        d.sort(function (a, b) {
          return self.sortSubgroups(subgroups[i][a].basevalue,
    ...
    if (self.sortChords) this.resort();
  }
