...
@Component({
    selector: 'app-dashboard-citations-count-profile',
    templateUrl: './dashboard-citations-count-profile.component.html',
    styleUrls: ['./dashboard-citations-count-profile.component.css']
})
export class DashboardCitationsCountProfileComponent implements OnInit, OnChanges {
        ...
        svg.selectAll('circle')
            ...
            .on('click', function (d) {
                ...
                const searchWord = '*:* AND omics_type:"'
                    + DashboardCitationsCountProfileComponent.getName(d['year'], d['value'], annualDataExtends)
    ...
    private initSvg(body: any): any {
        ...
        const latestDatasetsDivHeightPx = d3.select('#barchart_citations_dashboard').style('height');
        ...
        const svg = d3.select('#barchart_citations_dashboard').append('svg')
