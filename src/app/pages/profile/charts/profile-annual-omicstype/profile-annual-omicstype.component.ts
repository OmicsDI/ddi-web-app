...
@Component({
    ...
    templateUrl: './profile-annual-omicstype.component.html',
    styleUrls: ['./profile-annual-omicstype.component.css']
})
export class ProfileAnnualOmicstypeComponent implements OnInit, OnChanges {
    ...
    private drawGraph(processedData: any): void {
        ...
        const xAxis = d3.axisBottom(x0).ticks(new Date().getFullYear() - (minDate.getFullYear())).tickFormat(d3.timeFormat('%Y'));
        ...
        svg.selectAll('circle')
            ...
            .on('mouseover', function (d: any, i: number) {
                ...
                const barchart_omicstype_annual = d3.select('#barchart_omicstype_annual').style('height');
                const barchart_omicstype_annual_height = barchart_omicstype_annual.substring(
                    0, barchart_omicstype_annual.indexOf('px'));
                ...
                const position = Number(profile_div_height) - Number(barchart_omicstype_annual_height) -
            ...
            .on('click', function (d) {
                ...
                const searchWord = '*:* AND omics_type:"'
                    + ProfileAnnualOmicstypeComponent.getName(d['year'], d['value'], annualDataExtends)
    ...
    private initSvg(body: any): any {
        ...
        const latestDatasetsDivHeightPx = d3.select('#barchart_omicstype_annual').style('height');
